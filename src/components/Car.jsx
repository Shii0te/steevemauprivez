...
export const Car = forwardRef(function Car(
  ...
  const exitCar = useCallback(() => {
    ...
    onExitPose?.({
      ...
      quaternion: carRef.current.quaternion.clone()
    })
  ...
  // reset états quand on (ré)active la voiture
  useEffect(() => {
    if (!enabled) {
  ...
  useEffect(() => {
    const down = (e) => {
      ...
      if (e.code === 'KeyE') {
        exitCar() // ✅ pas de double setControlMode
      }
    }
  ...
  useFrame((_, delta) => {
    ...
    // ✅ Sol : fallback
    const groundTargets =
      groundColliders && groundColliders.length > 0 ? groundColliders : worldScene.children
    ...
    const usedY = groundedCount > 0 ? sumGroundY / groundedCount : lastGround.current.avgGroundY
    const usedNormals = groundNormals.length > 0 ? groundNormals : lastGround.current.normals
    ...
    if (wallHits.length && wallHits[0].distance < 1.4) {
      ...
      if (now - lastWallHit.current > WALL_COOLDOWN) {
        ...
        if (hit.face) {
          vHitNormal.current.copy(hit.face.normal).transformDirection(hit.object.matrixWorld).normalize()
